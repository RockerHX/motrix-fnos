...
| P1-4 | 抽取 database | `server/` 承载数据库连接、schema 与 repository，`src-tauri` 仅保留 Tauri 路径适配 | `server/src/database` 中不出现 `tauri::Manager`，双轨测试通过 | `refactor: 抽取服务端数据库核心模块` | 已完成 | `refactor: 抽取服务端数据库核心模块` |
