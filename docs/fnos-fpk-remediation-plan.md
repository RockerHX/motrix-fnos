...
| P1-7 | 抽取无 Tauri 的 ServerState | `server/` 新增 `ServerState`，`src-tauri::AppState` 改为 Tauri 适配层 | `server/src/state` 中不出现 `tauri_plugin_shell`、`CommandChild` | `refactor: 抽取无Tauri依赖的ServerState核心` | 已完成 | `refactor: 抽取无Tauri依赖的ServerState核心` |
