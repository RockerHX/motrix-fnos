...
| P3-3 | 迁移基础服务到 HTTP | `app`、`aria2`、`settings`、`debug-logs` 服务层改为 HTTP；删除未使用的 `quitApp` 前端出口 | `pnpm run typecheck`、`pnpm run build` | `feat: 迁移基础服务到HTTP接口` | 已完成 | `feat: 迁移基础服务到HTTP接口` |
