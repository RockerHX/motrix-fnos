...
| P3-4 | 迁移任务服务并降级目录选择交互 | `tasks` 服务改为 HTTP；移除 Tauri 目录选择插件调用；任务新建弹窗改为手动输入保存路径 | `pnpm run typecheck`、`pnpm run build` | `feat: 迁移任务服务并降级目录选择交互` | 已完成 | `feat: 迁移任务服务并降级目录选择交互` |
