...
| P3-5 | 新增前端 SSE 运行时事件服务 | 用 `EventSource` 替换 Tauri `listen`；新增/重命名为 `runtimeEvents` 服务，消费 `tasks.snapshot` 与 `runtime.exiting` | `pnpm run typecheck`、`pnpm run build` | `feat: 新增前端SSE运行时事件订阅` | 已完成 | `feat: 新增前端SSE运行时事件订阅` |
