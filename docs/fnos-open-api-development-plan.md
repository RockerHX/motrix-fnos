...
> 状态：阶段 0 x86 核心验证完成并归档；阶段 1 P0 开发完成待正式身份实机验收；阶段 2 P1 路径与宿主体验进行中
>
...
状态：进行中。阶段 1 的正式 `motrix` 身份实机验收尚未完成，最终 FPK 将同时验收阶段 1 与阶段 2；这不改变阶段 1“开发完成，待实机验收”的准确状态。
...
完成标准：P1 能力不会改变下载安全边界，宿主不可用时页面仍可正常使用。
