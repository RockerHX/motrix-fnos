...
> 当前状态更新（2026-07-02）：阶段 0 至阶段 4 已完成，项目已可生成 x86 FPK。当前进入阶段 5 飞牛实机验证；OES / A311D 等 ARM 飞牛不能安装 x86 包，需构建 `aarch64-unknown-linux-gnu` 目标的 ARM FPK 后继续验证。本文档保留早期偏差审计内容，后续阶段状态以 `docs/development-plan.md` 和 `docs/fnos-manual-test-checklist.md` 为准。

## 1. 结论摘要

以下偏差评估是整改初期结论，已作为阶段 0 至阶段 4 的治理依据保留。当前项目已经完成主线纠偏和 FPK 打包链路，剩余重点是阶段 5 实机验证。

整改初期判断：当前项目**已经严重偏离最初目标的交付形态**。
...
本节记录整改初期为了成为可安装 FPK 而识别的缺失模块。阶段 4 已补齐 FPK 目录、server 构建、Web UI 输出、Aria2 sidecar、cmd 脚本、manifest 和打包入口；当前仅作为历史审计保留。
...
| P4-5.1 | 执行完整打包验证 | 生成并检查 `.fpk` 产物内容 | `pnpm run build:fpk` 成功生成 `packaging/fnos/dist/motrix.fnos_0.1.0_x86.fpk`，且包内包含 server、UI、sidecar、manifest、cmd | `fix: 修复FPK打包链路校验问题` | 已完成 | `fix: 修复FPK打包链路校验问题`（`600ea1f`） |
| P4-5.2 | 更新阶段 4 文档状态 | 同步整改计划与开发计划的阶段 4 完成状态 | 文档状态与代码状态一致，可进入阶段 5 | `docs: 收口阶段4FPK打包链路状态` | 已完成 | `docs: 收口阶段4FPK打包链路状态`（`0b59950`） |
...
阶段 0 已修正 `docs/architecture.md` 和 `docs/development-plan.md` 的主线偏差。后续风险转为：阶段状态、FPK 平台架构、实机验证结果必须及时同步到文档，否则会误导安装和排障。
