...
服务进程环境可能没有 `HOME` / `XDG_DATA_HOME`，通用 Linux 回收站机制无法被 fnOS 回收站 UI 可靠识别。当前未使用隐藏归档目录；删除任务勾选“同时删除本地文件”或重新下载时，会直接删除目标文件和同名 `.aria2` 控制文件。下载完成后同名 `.aria2` 控制文件也由 server 主动清理。
