...
后台驻留、退出清理、任务保存、Aria2 sidecar 启动和端口兜底策略详见 `docs/runtime-lifecycle-and-aria2-strategy.md`。架构层面的固定原则：

- 左上角关闭窗口只隐藏到后台，不退出应用，不停止 Aria2。
- 明确退出必须走统一退出流程：暂停未完成任务、持久化状态、停止当前 AppState 管理的 sidecar。
- Aria2 RPC 地址必须来自运行时 AppState，不能硬编码 6800。
- 只能清理确认属于本应用的残留 sidecar；外部端口占用应切换备用端口或报错。
