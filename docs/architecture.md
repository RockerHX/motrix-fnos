...
- 磁力链接任务遵循“先解析、后创建真实下载”的交互：后端先添加临时 magnet metadata 任务，metadata 完成后读取 `followedBy` 对应 BT 任务的文件列表与 Aria2 保存的 hash 命名 `.torrent`，随后移除临时 BT GID；前端阻塞展示文件确认，用户确认后后端再使用已保存 `.torrent` 和 `select-file` 创建并启动真实 BT 下载任务。磁链解析与真实下载都使用任务专属子目录，`.torrent` 作为确认后创建真实任务的输入及排障产物；解析阶段不得把真实 BT 任务长期保留为“暂停中的下载任务”。
