...
- 磁力链接任务遵循 Aria2 原生 metadata 流程：先添加 magnet metadata 任务，metadata 完成后通过 `followedBy` 切换到真实 BT GID；真实任务保持暂停并由前端展示文件列表，用户确认后后端再设置 `select-file` 并恢复下载。磁链任务也会使用任务专属子目录，并启用 Aria2 `bt-save-metadata` 将解析出的 hash 命名 `.torrent` 放入该目录，作为可见性 / 排障产物，不替代 Aria2 session 机制。
