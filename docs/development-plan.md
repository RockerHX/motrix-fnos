...
- 打包脚本可输出 x86 与 ARM FPK：
  - `packaging/fnos/dist/motrix.fnos_0.1.1_x86.fpk`
  - `packaging/fnos/dist/motrix.fnos_0.1.1_arm.fpk`
...
- x86 设备安装 `motrix.fnos_0.1.1_x86.fpk`。
- `aarch64` / `arm64` 设备安装 `motrix.fnos_0.1.1_arm.fpk`。
...
目标：把设置页中“真实生效”和“仅保存配置”的能力区分清楚，并补齐侧栏 Help 入口，避免用户误以为未实现的 fnOS 系统能力已经可用。

现状判断：

- 默认下载目录已生效：从 fnOS 已授权目录读取并保存，新建任务会优先使用该默认目录。
- 最大并发下载数、下载限速、上传限速已生效：保存后会通过 Aria2 RPC 应用到全局下载配置；RPC 未就绪时下次启动生效。
- 后台驻留当前只是说明项：服务持续运行由 fnOS 生命周期和 server 进程负责，Web UI 不直接控制。
- 开机自启当前仅保存配置字段，不会修改 fnOS 系统开机自启状态。
- 下载通知当前仅保存配置字段，不会申请浏览器通知权限，也未接入 fnOS 系统通知能力。
- `Help` 侧栏按钮当前未绑定点击事件，没有本地帮助页面或弹窗。

功能范围：

1. 设置页能力状态收口
   - 保留默认下载目录、最大并发下载数、下载限速、上传限速的现有生效逻辑。
   - 将开机自启和下载通知明确展示为“待支持 / 当前仅保存偏好”，默认禁用开关，避免误导用户。
   - 不在本阶段实现真实 fnOS 开机自启控制、系统通知或浏览器通知授权。
2. Help 本地帮助入口
   - 侧栏 Help 按钮增加点击事件，沿用现有侧栏事件传递方式。
   - 新增本地帮助弹窗，不联网打开外部页面。
   - 帮助内容覆盖授权目录、默认下载目录、下载设置、Trash 永久删除、Extensions 当前状态和日志诊断入口。
3. 文档与查证记录
   - 记录设置项能力边界和 Help 验收项。
   - 记录 fnOS 文档查证结论：当前未确认应用内控制系统开机自启和系统通知 API；后续实现前必须查证官方资料或实机验证。
   - 当前参考资料为飞牛应用开放平台开发文档及其公开同步镜像；已确认 manifest、权限、资源、运行时环境和授权目录变量等基础能力，但未确认应用内自启开关或系统通知接口。
4. 回归验证
   - 设置页可打开并保存已生效下载设置。
   - Help 可打开和关闭。
   - Diagnostics、五个侧栏菜单、新建任务默认目录不回退。

验收标准：

- 设置页不会把开机自启、下载通知表达为已支持的系统能力；两项在未查证前保持待支持状态。
- 默认下载目录、最大并发下载数、下载限速、上传限速仍可保存并按现有逻辑生效。
- 点击 Help 有响应，可打开本地帮助弹窗并关闭。
- Help 内容不依赖外网，并覆盖授权目录、默认下载目录、下载设置、Trash 永久删除、Extensions 状态和日志诊断入口。
- Settings、Diagnostics、Downloading、Completed、Stopped、Trash、Extensions 和新建任务默认目录不回退。

验证：

- `rtk pnpm run typecheck`
- 浏览器手动验证 Settings、Help、Diagnostics、五个侧栏菜单和新建任务默认目录。
- Settings 手动验收项：默认下载目录可从授权目录选择；并发和限速可保存；开机自启和下载通知显示待支持且不声明会修改系统行为。
- Help 手动验收项：侧栏 Help 可打开和关闭；内容为本地说明，不依赖外部网络。

任务追踪：

- [x] 0.1 文档计划提交。
- [x] 1.1 设置页能力状态收口。
- [x] 2.1 Help 入口与本地帮助弹窗。
- [x] 3.1 文档说明与验收项补齐。
- [x] 4.1 回归与阶段状态更新。
...
- `rtk pnpm run typecheck` 通过。
- 设置页已将开机自启和下载通知展示为待支持状态，不再表达为已支持的 fnOS 系统能力。
- Help 侧栏入口已接入本地帮助弹窗；帮助内容覆盖授权目录、默认下载目录、下载设置、Trash 永久删除、Extensions 状态和日志诊断入口。
- CLI 环境未执行浏览器手动点击回归；需在下一次实机或浏览器验收中补录：Settings 可打开保存、Help 可打开关闭、Diagnostics 不回退、五个侧栏菜单不回退、新建任务默认目录不回退。
