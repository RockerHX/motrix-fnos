...
当前计划新增“关于页、版本检测与更新历史”能力；该能力只展示信息和引导手动更新，不改变 FPK 生命周期，也不执行应用内自动安装。后续工作仍应先确认是否影响以下边界：

- 是否新增或改变 HTTP / SSE / JSON-RPC 契约；关于页将新增 `/api/app/update-check` 并扩展 `/api/app/info`。
- 是否新增长期状态、数据库字段或迁移。
