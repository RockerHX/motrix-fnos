...
- 阶段 9 已完成应用内国际化：平台外壳保持中文，Web UI 支持在设置中手动切换简体中文 / English，并通过现有设置接口持久化。
...
目标：平台外壳保持固定中文，Web UI 自行提供中英文语言切换，并把语言偏好纳入现有设置持久化。

完成结论：

- FPK 平台外壳保持中文：`manifest`、`app/ui/config`、`wizard/*` 不做多语言。
- Web UI 已增加轻量 i18n 字典，不引入第三方 i18n 依赖。
- 设置页已增加“界面语言 / Language”下拉，支持 `zh-CN` 与 `en-US` 手动切换，保存后立即生效。
- `AppConfig` 已增加 `language` 字段，通过 `/api/settings` 读取和保存；旧配置缺少语言字段或非法语言值时回退 `zh-CN`。
- Naive UI 内置 locale / date locale 已随应用语言切换。
- 第一版覆盖侧栏、空态、新建任务、任务表格、任务操作、设置、帮助、扩展页、诊断和调试日志等前端可控文案；后端 / Aria2 原始消息、日志内容、路径、URL 和错误详情保持原样。

验收标准：

- 默认语言为简体中文。
- 设置中切换 English 后主界面、任务、设置、帮助、诊断等前端可控文案立即切换。
- 刷新页面后保持已保存语言。
- 切回简体中文后 UI 恢复中文。
- 平台外壳和安装向导仍保持中文。
