...
- 开机自启和下载通知不在设置页展示：当前未确认可用的 fnOS 系统接口，前端保存时会固定写回关闭状态。
...
1. 设置页能力状态收口
   ...
   - 设置页不展示开机自启和下载通知，避免不可用开关误导用户；前端保存时固定写回关闭状态。
...
- 设置页不展示开机自启、下载通知，不会把未支持系统能力表达为可操作功能。
...
- Settings 手动验收项：默认下载目录可从授权目录选择；并发和限速可保存；开机自启和下载通知不在设置页展示。
