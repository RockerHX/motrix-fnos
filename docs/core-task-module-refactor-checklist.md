...
| 2 | 抽离任务查询与持久化同步 | 已完成 | `refactor(tasks): 抽离任务查询与持久化同步` |
| 3 | 抽离任务删除与回收站流程 | 已完成 | `refactor(tasks): 抽离任务删除与回收站流程` |
| 4 | 抽离暂停、恢复与重新下载流程 | 已完成 | `refactor(tasks): 抽离任务控制流程` |
| 5 | 抽离普通 URL 与种子创建流程 | 已完成 | `refactor(tasks): 抽离 URL 与种子任务创建流程` |
| 6 | 抽离磁链解析与文件确认流程 | 已完成 | `refactor(tasks): 抽离磁链解析与文件确认流程` |
| 7 | 收口后端服务入口 | 已完成 | `refactor(tasks): 收口任务服务入口` |
