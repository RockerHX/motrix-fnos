...
pub(crate) async fn maintain_aria2_logs(
    ...
    maintain_aria2_log_files(&state.runtime.app_data_dir)
        .map(Aria2LogMaintenanceOutcome::Maintained)
}
...
fn remove_excess_history_logs(histories: Vec<HistoryLog>) -> Result<(u64, usize), String> {
    ...
    for history in histories.into_iter().skip(ARIA2_LOG_HISTORY_RETENTION) {
